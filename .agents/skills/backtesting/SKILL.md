...
Read `.Codex/knowledge/strategies/_index.md` to get the current strategy list and their statuses.
...
```bash
python3 .Codex/tools/backtest_strategy.py {strategy_id} --start 2025-01-01 --end 2026-03-31
```
...
```bash
python3 .Codex/tools/backtest_strategy.py {strategy_id} --start {chunk_start} --end {chunk_end}
```
...
If yes, update `.Codex/knowledge/strategies/_index.md` (Backtest Results Archive table) and the relevant `.Codex/knowledge/strategies/{id}.md` file with the new run date, period, and metrics.
