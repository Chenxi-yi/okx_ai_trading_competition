...
Read `.Codex/knowledge/strategies/_index.md` to get the current strategy list.
...
Wait for the user's answer before continuing. Only one strategy can run at a time — if the engine is already running, tell the user to stop it first with `python3 .Codex/tools/stop_engine.py`.
...
```bash
python3 .Codex/tools/trading_status.py
```
If running, stop here and tell the user: "Engine is already running. Run `python3 .Codex/tools/stop_engine.py` first."
...
3. **Balance check:**
```bash
python3 .Codex/tools/check_balance.py
```
...
```
python3 .Codex/tools/start_demo.py {strategy_id} --foreground
```

**IMPORTANT:** Do NOT use the Bash tool to run this command — it will block and the user won't see live output. Instead, display the command and instruct the user to run it themselves by typing `! python3 .Codex/tools/start_demo.py {strategy_id} --foreground` in the prompt (the `!` prefix pipes it to their terminal).
...
python3 .Codex/tools/check_positions.py
python3 .Codex/tools/trading_status.py
```
