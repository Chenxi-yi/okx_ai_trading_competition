...
python3 .Codex/tools/trading_status.py          # engine health check
python3 .Codex/tools/check_balance.py            # OKX account balance
python3 .Codex/tools/check_positions.py          # open swap positions
python3 .Codex/tools/backtest_strategy.py <id>   # run backtest
python3 .Codex/tools/place_order.py --instId BTC-USDT-SWAP --side buy --sz 1 --dry-run
python3 .Codex/tools/start_demo.py <id>          # start demo run
python3 .Codex/tools/stop_engine.py              # stop daemon
python3 .Codex/tools/log_error.py --code X --msg "..."  # error logging
```
...
/Users/yichenxi/Desktop/okx_ai_skill_challenage/
  ...
  .Codex/
    tools/                           Codex agent tools (see _registry.md for full docs)
...
| File | When to read |
|---|---|
| `.Codex/knowledge/strategies/_index.md` | Strategy comparison, status, backtest results |
| `.Codex/knowledge/strategies/<id>.md` | Full signal formulas, parameters, code pointers for one strategy |
| `.Codex/knowledge/okx_api.md` | Before any CLI call — instId, sz, profiles, pitfalls |
| `.Codex/knowledge/engine_architecture.md` | Before modifying engine code or running commands |
| `.Codex/knowledge/competition.md` | Scoring rules, eligibility, deadlines |
| `.Codex/knowledge/risk_system.md` | Risk thresholds, circuit breakers |

## Tools
**Full reference:** `.Codex/tools/_registry.md` — args, return format, examples, permissions.
...
Every error encountered → `python3 .Codex/tools/log_error.py --code X --msg "..." --context "{...}"`
Every error resolved → same command with `--context '{"resolved": true, "fix": "..."}'`
KB found stale or wrong → edit the relevant `.Codex/knowledge/` file directly
New API behaviour discovered → update `.Codex/knowledge/okx_api.md`
After backtest → update results table in `.Codex/knowledge/strategies/<id>.md`
